...
saliency_directory = f"dataset/{dataset_name}_atta_WB_map"
...
image_filenames = {os.path.splitext(os.path.basename(path))[0]: path for path in image_paths_1}
saliency_filenames = {os.path.splitext(os.path.basename(path))[0][22:-1]: path for path in saliency_paths_1}
...
image_paths_1, saliency_paths_1 = (
    list(t) for t in zip(*matching_pairs)
) if matching_pairs else ([], [])
# Check if any valid pairs exist
if not image_paths_1 or not saliency_paths_1:
    raise ValueError("No matching image-saliency pairs found.")
#
# train_images, val_images, train_saliency, val_saliency = train_test_split(
#     filtered_image_paths, filtered_saliency_paths, test_size=0.1, random_state=42
# )
...
# Get all image paths
image_paths_2 = glob.glob(os.path.join(image_directory, "*.[jp][pn]*g"))
saliency_paths_2 = glob.glob(os.path.join(saliency_directory, "*.[jp][pn]*g"))
#
# # Sort the lists to ensure correct pairing
image_paths_2.sort()
saliency_paths_2.sort()
#
# Check if the number of images and masks are equal
if len(image_paths_2) != len(saliency_paths_2):
    raise ValueError(f"Number of images ({len(image_paths_2)}) and masks ({len(saliency_paths_2)}) do not match.")


combined_image_paths = image_paths_1 + image_paths_2
...
#         for images, masks in loader:
#             images, masks = images.to(device), masks.to(device)
#             gen_masks = model(images)
#             loss = criterion(gen_masks, masks)
#             running_loss += loss.item() * images.size(0)
#     epoch_loss = running_loss / len(loader.dataset)
#     return epoch_loss
#
