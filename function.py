...
def optimize_poison(args, net, optimizer, nice_train_loader, epoch, writer, vis=args.vis):
    # eval mode
    net.eval()
    dataset = os.path.basename(args.data_path)
    points = []
    names = []
    n_val = len(nice_train_loader)  # the number of batch
    ave_res, mix_res = (0, 0, 0, 0), (0, 0, 0, 0)
    rater_res = [(0, 0, 0, 0) for _ in range(6)]
    hd = []
    tot = 0
    ...
    return tot / n_val, tuple([a / n_val for a in mix_res]), sum(hd) / len(nice_train_loader)